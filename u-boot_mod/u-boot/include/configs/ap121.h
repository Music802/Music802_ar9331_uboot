...
#elif defined(CONFIG_FOR_LINKCARD_MUSIC802)
	/* LEDs */
	#define CONFIG_QCA_GPIO_MASK_LEDS_ACTIVE_HI		(GPIO15 | GPIO16 | GPIO17)

	/* Outputs, inputs */
	#define CONFIG_QCA_GPIO_MASK_OUTPUTS			        CONFIG_QCA_GPIO_MASK_LEDS_ACTIVE_HI
	#define CONFIG_QCA_GPIO_MASK_INPUTS				GPIO26
...
#undef CONFIG_BOOTARGS
	...
	#define	CONFIG_BOOTARGS	"board=TL-WR703N console=ttyATH0,115200  root=1f02 mtdparts=spi0.0:256k(u-boot)ro,64k(u-boot-env)ro,2048k(kernel),13888k(rootfs),64k(nvram),64k(art)ro,15936k@0x50000(firmware)"
	//#define	CONFIG_BOOTARGS	"board=Music802 console=ttyATH0,115200  root=1f02 mtdparts=spi0.0:256k(u-boot)ro,64k(u-boot-env)ro,2048k(kernel),13888k(rootfs),64k(nvram),64k(art)ro,15936k@0x50000(firmware)"
...
#elif defined(CONFIG_FOR_8DEVICES_CARAMBOLA2) || \
	defined(CONFIG_FOR_LINKCARD_MUSIC802) || \
...
#elif defined(CONFIG_FOR_LINKCARD_MUSIC802)
	// Music802: 256k(U-Boot),64k(U-Boot env),64k(ART)
	#define WEBFAILSAFE_UPLOAD_LIMITED_AREA_IN_BYTES	(384 * 1024)
